 ...
 * Checker-Framework-Inference through a static method getInstance. InferenceMain uses the
 * InferrableChecker of the target checker to instantiate components and wire them together. It
 * creates and holds instances to the InferenceVisitor, the InferenceAnnotatedTypeFactory, the
 * InferrableChecker, etc.
 *
...
public class InferenceMain {
    ...
    private void writeJaif() {
        ...
        try (PrintWriter writer =
            ...
            if (solverResult == null) {
                ...
                // add any custom annotations that must be inserted to the JAIF header, such as
                // alias annotations
                for (Class<? extends Annotation> annotation :
                        realChecker.additionalAnnotationsForJaifHeaderInsertion()) {
                    annotationClasses.add(annotation);
                }
            }
    ...
    public SlotManager getSlotManager() {
        if (slotManager == null) {
            Set<? extends AnnotationMirror> tops =
                    realTypeFactory.getQualifierHierarchy().getTopAnnotations();
            if (tops.size() != 1) {
                throw new BugInCF(
                        "Expected 1 real top qualifier, but received %d instead", tops.size());
            }
    ...
    /**
     * Parse checker framework args from a space separated list of -Axxx=xxx,y=y -Azzz=zzz
     *
     * @return List of Strings, each string is a checker framework argument in the format:
     *     -Axxx=xxx,y=y or -Azzz or -Azzz=zzz
     */
    ...
    public abstract static interface ResultHandler {
