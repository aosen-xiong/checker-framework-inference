...
/**
 * Options for the InferenceLauncher and InferenceMain (though InferenceMain uses only the subset of
 * options that apply to inference).
 */
public class InferenceOptions {
    ...
    /**
     * The type system to use for checker, solver, and related command-line options. If you use this
     * option, all required command-line arguments except --mode will have values and the only other
     * option you need to include is a list of source files.
     *
     * <p>All legal options are listed in InferenceOptions.typesystems.keySet()
     */
    ...
    @Option(
            "[InferrableChecker] the fully-qualified name of the checker to run; overrides --typesystem.")
    public static String checker;

    @Option(
            "[InferenceSolver] the fully-qualified name of the solver to use on constraints; overrides --typesystem.")
    public static String solver;

    @Option(
            "The fully-qualified name of the classpath for target program; overrides --targetclasspath.")
    ...
    @Option(
            "The JSON file to which constraints should be dumped.  This field is mutually exclusive with solver.")
    ...
    @Option(
            value =
                    "Annotation file utilities output directory.  WARNING: This directory must be empty.",
            aliases = "-afud")
    ...
    public static File pathToThisJar =
            new File(CheckerMain.findPathTo(InferenceOptions.class, true));
    ...
    public static InitStatus init(String[] args, boolean requireMode) {
        ...
        if (typesystem != null) {
            ...
            if (spec == null) {
                errors.add(
                        "Unrecognized typesystem.  Current typesystems:\n"
                                + StringsPlume.join("\n", typesystems.keySet()));
            } else {
                spec.apply();
            }
        ...
        if (mode == null) {
                        ...
                        if (solverArgs == null || solverArgs.isEmpty()) {
                            ...
                            solverArgs =
                                    solverArgs
                                            + ","
                                            + "constraint-file="
                                            + InferenceOptions.jsonFile;
                        }
                    } else {
                        errors.add(
                                "You must specify a solver using --solver or a --jsonFile to write constraints in.");
                    }
            ...
            if (modeEnum != null && modeEnum.ordinal() >= Mode.ROUNDTRIP.ordinal()) {
                if (afuOutputDir == null) {
                    if (!inPlace) {
                        errors.add(
                                "You must specify an Annotation File Utilities output directory (--afuOutputDir or -afud) or --inPlace.");
                    }
                } else if (inPlace) {
                    errors.add(
    ...
    static {
        ...
        typesystems.put(
                "ostrusted",
                new TypeSystemSpec(
                        OsTrustedChecker.class,
                        MaxSat2TypeSolver.class,
                        new File(srcDir, "ostrusted" + File.separator + "jdk.astub")));
        typesystems.put(
                "interning",
                new TypeSystemSpec(InterningChecker.class, MaxSat2TypeSolver.class, null));
        typesystems.put(
                "sparta-source",
                new TypeSystemSpec(
    ...
    private static class TypeSystemSpec {
        ...
        private TypeSystemSpec(
                Class<? extends InferenceChecker> qualifiedChecker,
                Class<? extends InferenceSolver> defaultSolver,
                File defaultStubs) {
        ...
        private TypeSystemSpec(
                ...
                Class<? extends InferenceSolver> defaultSolver,
                File defaultStubs,
                String[] defaultJavacArgs,
                String defaultSolverArgs) {
